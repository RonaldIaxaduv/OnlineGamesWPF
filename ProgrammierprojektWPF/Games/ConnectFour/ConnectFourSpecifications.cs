...
{
    [Serializable]
    public class ConnectFourSpecificationsPortable
    {
        ...
        public Size BoardSize
        {
            ...
            private set
            {
                if (value.Width < 4 && value.Height < 4) throw new ArgumentException("Connect four boards need to be at least 4 units wide or tall.");
                boardSize = value;
            }
        }
        ...
        public string[] PlayerNames
        {
            ...
            private set
            {
                if (value.Length != 2) throw new ArgumentException("Connect Four can only be played by 2 players.");
        ...
        public ConnectFourSpecificationsPortable(Size boardSize, string[] playerNames, int first)
        {
            BoardSize = boardSize;
    ...
    public class ConnectFourSpecifications
    {
                    ...
                    case Game.GameLocation.Server:
                        if (Players.Length > 0)
                        {
                            foreach (Player player in Players)
                            {
                                ...
                                { throw new ArgumentException("Local non-computer users are not supported for Connect Four servers."); }
                        ...
                        { throw new ArgumentException("Connect Four server require a server wrapper."); }
                        break;
                    ...
                    case Game.GameLocation.Client:
                        if (Players.Length > 0)
                        {
                            foreach (Player player in Players)
                            {
                                ...
                                if (player.type == Player.playerType.LocalHuman && player.connectionToServer == null)
                                { throw new ArgumentException("All local users need a connection to the client."); }
                            }
                        }
                        if (clientWrapper == null)
                        { throw new ArgumentException("Connect Four clients require a client wrapper."); }
                        break;
                    ...
                    case Game.GameLocation.Local:
                        if (Players.Length > 0)
                        {
                            foreach (Player player in Players)
                            {
                                ...
                                { throw new ArgumentException("Remote users are not supported for local Connect Four games."); }
                            }
                        }
                        break;
        ...
        public Size BoardSize
        {
            ...
            private set
            {
                if (value.Width * value.Height <= 4) throw new ArgumentException("Connect Four boards need to consist of at least 4 fields.");
                boardSize = value;
            }
        }
        ...
        public ConnectFourSpecifications(Game.GameLocation gameLocation, Size boardSize, Player[] players, int first, Client clientWrapper = null, Server serverWrapper = null)
        {
            BoardSize = boardSize;
            Players = players;
            First = first;
        ...
        public ConnectFourSpecifications(ConnectFourSpecificationsPortable pSpecs, Game.GameLocation gameLocation, Player[] players, Client clientWrapper = null, Server serverWrapper = null)
        {
